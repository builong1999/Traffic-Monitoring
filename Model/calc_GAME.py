...
class caclGAME:
    def __init__(self,groundtruthPath,predictionPath):
        self.groundtruthPath = [item for item in os.listdir(groundtruthPath) if ".jpg" in os.path.basename(item)]
    ...
    def GAME_recursive(self,pre, gth, level, currentLevel = 0):
        if currentLevel == level:
            ...
            return sum(res)
    def Start(self):
        totalFile = len(self.groundtruthPath)
        print('Totalfile: {val:d}'.format(val = totalFile))
        mae, game1, game2, game3, game4 = 0
        for file in self.groundtruthPath:
            fileName = os.path.basename(file).split('.')[0] + '.h5'
            arrGT = np.array(h5py.File(file, "r")['density'])
            arrPre = np.array(h5py.File(join(self.predictionPath,fileName), "r")['density'])
...
c = caclGAME('E:/groundtruth','E:/prediction')
c.Start()